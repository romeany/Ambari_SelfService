...
class Master(Script):
    ...
    def install(self, env):
        ...
        #cmd = "cd /var/www/html/ambari/;cp elasticsearch-5.4.3.rpm elasticsearch.rpm"
        Execute(cmd)

        # Install Elasticsearch
        #cmd = "cd /var/www/html/ambari; rpm -Uvh elasticsearch.rpm"
        #cmd = "cd /var/www/html/ambari; rpm --install elasticsearch.rpm"
        cmd = format("cd {elastic_download_dir}; rpm --install elasticsearch.rpm")
        Execute(cmd)
        ...
        cmd = format("chown -R elasticsearch {elastic_conf_dir}")
        Execute(cmd)
